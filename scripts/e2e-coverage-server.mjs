...
// SAY WHY IT DIED. Without this, `EADDRINUSE` — a port this suite's own
// previous run failed to release, which `e2e-local.sh` has had to guard
// against before — surfaces as an uncaught exception with a raw stack and no
// coverage file. That is the silent failure this whole file exists to
// prevent, so it gets the same prefix and a non-zero exit as everything else.
server.on("error", (error) => {
  const detail =
    error.code === "EADDRINUSE"
      ? `:${port} is already in use — a previous run has not released it`
      : String(error);
  console.error(`[e2e-coverage] server failed: ${detail}`);
  process.exit(1);
});
