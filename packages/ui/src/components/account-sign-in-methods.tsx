...
} from "@quagga/ui/components/account-change-password";

// Sign-in methods list (canvas SjInE §"Sign-in methods"), shared by all three
// apps: password, Google, passkeys.
//
// What is REAL here and what is not, per the capability probe:
//  • Password change — SUPPORTED. The inline form genuinely changes it.
...
//    has none. Rather than a form that would always fail, the row says so.
//  • Unlink — exposed on the browser client but absent from the server surface
//    and unverifiable server-side. The button is disabled and carries the
//    capability's own message, passed in by the app.
//  • Passkeys — REAL (self-hosted @better-auth/passkey, migration 0015). They
//    are managed on the Security page, so this row links there.
//
// The last-method rule is shown as a note AND enforced server-side in
// @quagga/core (`canUnlinkSignInMethod`, `assessDeletionEligibility`) — this
// list is not the security boundary.
...
export interface SignInMethodsProps {
  ...
  methodCount: number;
  /** Where this app's Security page lives (apps mount the suite at their own paths). */
  securityHref: string;
  /**
   * The unlink capability's own words, resolved from @quagga/core by the app.
   * Shown on the disabled button and in the closing note.
   */
  unlinkNotice: string;
  /** Password policy, injected — see AccountChangePassword. */
  ...
  return (
    <div className="flex flex-col">
      ...
      <MethodRow
        ...
        action={
          googleLinked ? (
            <Button variant="ghost" size="sm" disabled title={unlinkNotice}>
              Unlink
            </Button>
          ) : null
        }
