...
//
// THIS FILE IS THE SOURCE OF TRUTH; MIGRATIONS ARE NOW HAND-AUTHORED.
// `pnpm db:generate` is NOT safe to run against this schema and has not been
// used since 0023. The snapshot chain under migrations/meta/ stops at
// `0023_snapshot.json`, so drizzle-kit diffs this file against a picture of the
// database that predates 0024–0029 and emits a migration re-creating
// `wrangler_assignments` and re-adding `decision_reason`, `consent_edition_id`
// and `questionnaire_responses.group_id`. Against the live database that fails
// at the first CREATE TABLE — and this project's migrations run on deploy.
//
// So: write the migration by hand, in the style of 0024–0029 (defensively
// idempotent — `IF NOT EXISTS`, `DO $$ … pg_constraint` guards — and commented
// with the reasoning), and add its `_journal.json` entry. Restoring the snapshot
// chain is a separate piece of work; until it happens, treat `db:generate` as
// broken rather than as the default path.
//
