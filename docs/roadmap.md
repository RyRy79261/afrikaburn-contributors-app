...
- **Placement & layout tooling** — **explicitly deferred**: no structured geo data exists, the official map is a PDF that arrives late, and the layout changes every year — nothing reliable to build against. Staff-assigned codes/locations on camp profiles (R1) cover what other workflows need. The layout designer / erf-fit ideas stay parked until AB's map process changes. _(16 Sep 2026: AB has indicated a **map API** is coming — the "map process changes" clause. Still deferred, because an announced API is not a read one; a research plan now exists in [`gis-placement-spec.md`](gis-placement-spec.md), whose Phase 0 is a read-only probe that costs nothing downstream and produces the written contract the rest would need.)_
...
| Pricing (routes/storage/water/ice/gas) and refund policy. **How** AB collects is no longer open — [Decision 009](decisions/decision-009-payment-direction.md) settles it: their channels, our references, our status record. What remains is the amounts, and they apply to logistics only — registration is free | R3 | AB |
| Site map / erf data format — **an API is now promised; we need a URL, credentials and one example payload** ([plan](gis-placement-spec.md))                  | Placement candidate work                   | AB            |
