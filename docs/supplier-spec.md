...
*Ryan, 24 Jul 2026. Suppliers get their own portal with a real onboarding; the org sees
...
(ab_sheet/manual) is dead — remove it everywhere.*
...
- Landing/sign-in → onboarding checklist page: each step as a card with its
  ...
  so suppliers *read the rules where they act on them*).
